...
void Projection::setScaleAndCenter(const Waypoint& center_wpt,
                                   ...
                                   unsigned int drawing_dist_at_max_range)
{
    ...
    // set new translation
    convertLatLonToXY(center_wpt.pointLatLon(), m_center_xy);

    // test the new center
    //QPointF center_xy_test_wpt;
    //convertLatLonToXY(center_wpt.pointLatLon(), center_xy_test_wpt);
    ...
    convertLatLonToXY(pbd_wpt.pointLatLon(), max_range_wpt_xy);
...
bool Projection::convertLatLonToXY(const QPointF& latlon_point, QPointF& xy_point) const
{
    //latlon_point.x() > -90.0 && latlon_point.x() < 90.0;
    //latlon_point.y() >= -180.0 && latlon_point.y() <= 180.0;
...
bool Projection::convertXYToLatLon(const QPointF& xy_point, QPointF& latlon_point) const
{
    // translate and scale
